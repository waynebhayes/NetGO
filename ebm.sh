...
USAGE="USAGE: $BASENAME [data file]
PURPOSE: compute holistic p-value of possibly correlated variates using Empirical Brown's Method
    input: m lines, with n+1 columns each.
    Each line represents a variable; the first entry is the p-value, and then the n raw samples of that variable.
...
hawk 'NR==1{nCols=NF}
	{
	    ASSERT(NF==nCols,"number of columns must be constant; first column had "nCols" but this one has "NF);
	    pVal[NR]=$1; for(i=1;i<NF;i++) data[NR][i]=$(i+1)
	}
    END{
	m=NR
	# Compute covariances across all samples of all input variables.
	for(i=1;i<=m;i++) for(j=1;j<=m;j++)for(k=1;k<NF;k++) CovarAddSample(i" "j, data[i][k], data[j][k]);
	# Now perform Empirical Browns Method
	df_fisher = Expected = 2.0*m;
	cov_sum = 0;
	for(i=1;i<=m;i++) for(j=i+1;j<=m;j++) cov_sum += CovarCompute(i" "j);
	Var = 4.0*m+2*cov_sum;
	c = Var/(2.0*Expected);
	...
	x *= 2;
	p_brown = Chi2_pair(int(df_brown+0.5), 1.0*x/c)
