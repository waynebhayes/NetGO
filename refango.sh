...
if $EBM; then
    ...
    awk '/^GO:/{pV[$1]=$NF}/\tGO:/{n=split($2,a,",");for(i=1;i<=n;i++)GOpair[a[i]][$1]=pair[$1]=1}END{printf "GOterm\tpValue"; for(p in pair)printf "\t%s",p; print ""; for(g in GOpair){printf "%s\t%g",g,pV[g]; for(p in pair)printf "\t%d",1*GOpair[g][p]; print ""}}' $TMPDIR/GOeval.out | tee $TMPDIR/ebm.in | ebm.sh || trap "" 0 1 2 3 15
fi
